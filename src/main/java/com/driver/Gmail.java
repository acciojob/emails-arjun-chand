...
public class Gmail extends Email {
    ...
    public void receiveMail(Date date, String sender, String message){
        ...
        if(!inbox.isEmpty() && (date.compareTo(inbox.getLast().getDate()) >= 0)) {
    ...
    public void deleteMail(String message){
        ...
        for(Mail mail : inbox){
            if(mail.getMessage().contains(message)) {
                inbox.remove(mail);
                trash.add(mail);
            }
        }
    ...
    public int findMailsBetweenDates(Date start, Date end){
        ...
        int count = 0;
        for(Mail mail : inbox){
            if(start.compareTo(mail.getDate()) >= 0 && end.compareTo(mail.getDate())<=0)
                count++;
        }
        return count;
    }
